...
const STRINGS = {
  ko: {
    ...
    typeSub: "같은 1KB라도 타입 시그니처를 randomized bucket으로 분리",
    typeVerdict: "다른 bucket의 타입은 같은 슬롯 재활용이 어려움 → 재할당 후보를 확률적으로 축소 (완전 차단은 아님)",
    poolLabel: "kmalloc-1k",
    zoneA: "bucket A (type A…)",
    zoneB: "bucket B (type B…)",
    victim: "취약 객체",
    attacker: "공격자 객체",
    slub: [
  ...
  en: {
    ...
    typeSub: "Even at 1KB, type signatures are split into randomized buckets",
    typeVerdict: "Types in other buckets rarely reuse the same slot → shrinks realloc candidates (probabilistic, not a hard block)",
    poolLabel: "kmalloc-1k",
    zoneA: "bucket A (type A…)",
    zoneB: "bucket B (type B…)",
    victim: "victim",
